...
- Do NOT read `.env`/secret files directly (a `PreToolUse` hook hard-denies it). Every `config:show`/`config:get` needs the user's explicit approval (the hook asks) - config reads resolve env values, so no key is auto-classified as safe. For a secret, get the key NAME from `.env.example` and ask the user. See the "Secrets" note in the temari rules above.
